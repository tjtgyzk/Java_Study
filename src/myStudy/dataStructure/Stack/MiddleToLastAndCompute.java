...
public class MiddleToLastAndCompute {
    public static void main(String[] args) {
        ...
        while (!oper.isEmpty()) {
            res.push(oper.pop());
            String s3 = res.pop().toString();
            int num1 = Integer.valueOf(res.pop().toString());
            int num2 = Integer.valueOf(res.pop().toString());
            res.push(compute(s3, num1, num2));
    ...
    public static void kuoHao(String kuohao, Deque oper, Deque res) {
        if (kuohao.equals("(")) {
            ...
            while (!oper.peekFirst().equals("(")) {//直到遇到左括号为止，弹出符号栈的符号，压入结果栈
                res.push(oper.pop());
                String s = res.pop().toString();
                int num1 = Integer.valueOf(res.pop().toString());
                int num2 = Integer.valueOf(res.pop().toString());
                res.push(compute(s, num1, num2));
            }
    ...
    public static String compute(String oper, int num1, int num2) {
        Integer res = 0;
        switch (oper) {
            case "+":
                res = num2 + num1;
